...
@dataclass
class ProfileResult:
    """Data generated by a model profiling
    """
    # in ms:
    avg_cpu_time : float 
    # amount of flops
    flops : float 
    # Amount of signs evaluated at the same time
    batch_size : int
    # amount of model calls to compute this data
    n_calls : int
    # in mb
    model_size : float 
...
class Trainer:
    ...
    def profile(self) -> ProfileResult:
        ...
        old_device = next(self._model.parameters()).device
        self._model.to("cpu")
        ...
        with torch.no_grad():
            ...
            for d in data:  
                ...
                cpu_times.append(averages.self_cpu_time_total)
                flop_counts.append(averages.total_average().flops)

        
        # Compute model size in memory
        param_size = 0
